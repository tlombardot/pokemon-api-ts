...
export class PokemonEvolution extends HTMLElement{
    ...
    private async loadEvolutions(speciesUrl:string) {
        ...
        try {
            ...
            container.className = "flex items-center justify-center overflow-x-auto py-4";
    ...
    private buildEvoTree(chainNode: any): EvoNode {
        ...
        if (chainNode.evolves_to && chainNode.evolves_to.length > 0) {
            node.evolvesTo = chainNode.evolves_to.map((childNode: any) => this.buildEvoTree(childNode));
        }
