...
export async function updateFilter(currentList:PokemonURL[], fullList:PokemonURL[],index:number,limit:number){
    ...
    if(typeValue != "all") {
        ...
        currentList = currentList.filter((p:PokemonURL) => typeSet.has(p.name.toLowerCase()))
    }
    if(generationValue != "all"){
        ...
        currentList = currentList.filter((p:PokemonURL) => generationSet.has(p.name.toLowerCase()))
    }
    if (abilitiesValue != "all"){
        ...
        currentList = currentList.filter((p:PokemonURL) => abilitiesSet.has(p.name.toLowerCase()))
    }
    indexValue = 0;
...
export async function typesSelector(currentList:PokemonURL[], fullList:PokemonURL[], index:number, limit:number) {
...
export async function generationSelector(currentList:PokemonURL[], fullList:PokemonURL[], index:number, limit:number) {
...
export async function abilitySelector(currentList:PokemonURL[], fullList:PokemonURL[], index:number, limit:number) {
